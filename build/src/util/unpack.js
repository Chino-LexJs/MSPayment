...
function unpack_ISO(message) {
    ...
    let primaryBitmapBinary = (0, hexa_bin_1.hex2bin)(primaryBitmap);
    ...
    if (primaryBitmapBinary[0] === "1") {
        let secondaryBitmapBinary = (0, hexa_bin_1.hex2bin)(message.substr(20, 16));
    ...
    for (const key in fields) {
        if (arrayOfCampos.includes(Number(fields[key][SUB_CAMPO_DATA_ELEMENT]))) {
            let longitud = longitudParam(key, message, init);
 ...
 */
function longitudParam(key, message, init) {
    switch (key) {
