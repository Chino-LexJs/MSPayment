  ...
  if (type.typeName && type.typeName.escapedText) {
    ...
    if (type.typeArguments && type.typeArguments.length) {
      const args = type.typeArguments
        .map((subType) => getTypeName(subType, src))
        .join(", ");
...
const getName = (node, src) => {
  let name =
    ...
    (node.parameters &&
      src.substring(node.parameters.pos, node.parameters.end));
  ...
  typesToCheck.forEach((type) => {
    ...
    (type.members || [])
      ...
      .forEach((member) => {
        ...
        let typeName = members.length
          ? "object"
          : getTypeName(member.type, src);
        if (parentName) {
        ...
        jsDoc = appendComment(
          jsDoc,
          `@property {${typeName}} ${nameToPlace}   ${comment}`
        );
      ...
      if (jsDocNode) {
        ...
        if (ts.isInterfaceDeclaration(statement)) {
          ...
          statement.members.forEach((member) => {
            ...
            let memberComment = src.substring(
              member.jsDoc[0].pos,
              member.jsDoc[0].end
            );
            let memberName = getName(member, src);
            memberComment = appendComment(
              memberComment,
              [`@name ${name}#${memberName}`].join("\n")
            );
        ...
        if (ts.isClassDeclaration(statement)) {
          ...
          statement.members.forEach((member) => {
            ...
            const modifiers = (member.modifiers || []).map((m) =>
              m.getText({ text: src })
            );
